...
class Neural:
	...
	def split_data(self, percent):
		...
		self.input_test = np.array(df_input.sample(frac=0.5))
		self.output_test = np.array(df_output.sample(frac=0.5))

		if percent != 100:
			self.input_validation = self.input_data[int(split_at):]
			self.output_validation = self.output_data[int(split_at):]
	...
	def train_network(self, verbose=False, shuffle=True, validate=True, epo=30):
		...
		if validate:
			if self.input_validation is None or self.output_validation is None:
				print("Nie zainicjowano danych walidujących")
				raise ValueError("Nie zainicjowano danych walidujących")
			try:
				self.network.train(self.input_train, self.output_train, self.input_validation, self.output_validation, epochs=epo)
			except Exception:
